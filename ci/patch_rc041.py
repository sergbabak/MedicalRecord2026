...
for p in (root / 'src/MedicalRecord.Desktop').rglob('*.axaml'):
    ...
    if 'x:CompileBindings=' not in s:
        needle = 'xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"'; i = s.find(needle)
        if i >= 0:
            end = i + len(needle); s = s[:end] + '\n             x:CompileBindings="False"' + s[end:]
    p.write_text(s, encoding='utf-8')
...
for rel in ['src/MedicalRecord.Api/Endpoints/PapEndpoints.cs', 'src/MedicalRecord.Api/Endpoints/DocumentEndpoints.cs']:
    p = root / rel; p.write_text(p.read_text(encoding='utf-8').replace('details=', 'details:'), encoding='utf-8')

# macOS Development is intentionally unsigned; Production alone uses Developer ID/notarization.
p = root / 'packaging/macos/build-installer.sh'
s = p.read_text(encoding='utf-8')
s = s.replace('APP="$OUT/Medical Record 2026-$RID.app"', 'APP="$OUT/Medical Record 2026.app"')
start = s.index('sign_native(){'); end_marker = 'codesign --verify --strict --verbose=2 "$APP"\n'; end = s.index(end_marker, start) + len(end_marker)
